...
CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, printer_id TEXT NOT NULL, viewer_id TEXT NOT NULL, offer TEXT NOT NULL, answer TEXT, created_at INTEGER NOT NULL, expires_at INTEGER NOT NULL);
CREATE INDEX IF NOT EXISTS snapshots_printer_time ON snapshots(printer_id, captured_at DESC);
CREATE INDEX IF NOT EXISTS sessions_printer_expiry ON sessions(printer_id, expires_at);`);
...
export function dashboardData(): PrinterView[] {
  return (db.prepare("SELECT * FROM printers ORDER BY id").all() as Record<string, unknown>[]).map((row) => {
...
export function latestSnapshotPaths(printerId: string): string[] { return (db.prepare("SELECT path FROM snapshots WHERE printer_id=? ORDER BY captured_at DESC LIMIT 2").all(printerId) as {path:string}[]).map((row) => row.path); }
export function saveAnalysis(printerId: string, analysis: Analysis) { db.prepare("UPDATE printers SET previous_analysis_json=analysis_json, analysis_json=? WHERE id=?").run(JSON.stringify(analysis), printerId); db.prepare("UPDATE snapshots SET analysis_json=? WHERE id=(SELECT id FROM snapshots WHERE printer_id=? ORDER BY captured_at DESC LIMIT 1)").run(JSON.stringify(analysis), printerId); }
export function latestMediaPath(printerId: string): string | null { return (db.prepare("SELECT snapshot_path AS path FROM printers WHERE id=?").get(printerId) as {path?:string} | undefined)?.path ?? null; }

export function createSession(session: { id:string; printerId:string; viewerId:string; offer:string }) { db.prepare("DELETE FROM sessions WHERE expires_at < ?").run(Date.now()); db.prepare("INSERT INTO sessions VALUES (?, ?, ?, ?, NULL, ?, ?)").run(session.id, session.printerId, session.viewerId, session.offer, Date.now(), Date.now()+120_000); }
export function sessionAnswer(id:string, viewerId:string): string | null { return (db.prepare("SELECT answer FROM sessions WHERE id=? AND viewer_id=? AND expires_at>?").get(id, viewerId, Date.now()) as {answer?:string}|undefined)?.answer ?? null; }
export function pendingSessions(printerId:string) { return db.prepare("SELECT id, offer FROM sessions WHERE printer_id=? AND answer IS NULL AND expires_at>? ORDER BY created_at").all(printerId, Date.now()) as {id:string;offer:string}[]; }
export function answerSession(id:string, printerId:string, answer:string) { const result=db.prepare("UPDATE sessions SET answer=? WHERE id=? AND printer_id=? AND expires_at>?").run(answer,id,printerId,Date.now()); if(result.changes!==1) throw new Error("Session not found or expired"); }
