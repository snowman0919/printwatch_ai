  ...
  return (
    <div className="relative aspect-video overflow-hidden bg-black">
      ...
      <div className="absolute inset-x-0 top-0 flex items-center justify-between bg-gradient-to-b from-black/70 to-transparent p-4">
        ...
        <button onClick={() => setPlaying((value) => !value)} className="flex h-9 items-center gap-2 border border-white/20 bg-black/50 px-3 text-xs font-medium text-white backdrop-blur">{playing ? <Pause size={14} /> : <Play size={14} />}{playing ? "일시정지" : "재생"}</button>
      </div>
    </div>
  );
}
